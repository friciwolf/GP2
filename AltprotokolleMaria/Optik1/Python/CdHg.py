...
#Histogramm
histogrammdaten=322.5+np.array([14,14,14,14,13,13,12,12,13,14])/60
figure()
hist(histogrammdaten,[322.5+11.5/60, 322.5+12.5/60, 322.5+13.5/60, 322.5+14.5/60])
xticks((322.5+12/60, 322.5+13/60, 322.5+14/60))
errorbar((np.mean(histogrammdaten)), (2.5), xerr=(phistd), yerr=(0.2), capsize=5, elinewidth=5)
text(np.mean(histogrammdaten), 1.5, '$\\sigma_{{\\psi}}$={:.4f}°\n$\\bar{{\\psi}}$={:.3f}°'.format(phistd, np.mean(histogrammdaten)), color='orange', fontsize=12)
xlabel('$\\psi$ in °')
